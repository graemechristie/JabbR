...
namespace JabbR.Test
{
    ...
    {
        static string[] getPartsForCommand(string command)
        {
        ...
        public class NickCommandFacts
        {
            public NickCommand Command
            {
                get
                {
                    var repository = new InMemoryRepository();
                    var chatService = new ChatService(repository, new Mock<ICryptoService>().Object);
                    var mockNotificationService = new Mock<INotificationService>();
                    return new NickCommand(mockNotificationService.Object, repository, chatService);
                }
            } 
            ...
            {
                var parts = CommandFacts.getPartsForCommand("/nick");

                Assert.Throws<InvalidOperationException>(() => Command.Handle(parts, "1", "lobby", "clientid", "test"));
            ...
            {
                var parts = CommandFacts.getPartsForCommand("/nick dfowler");
            ...
            {
                var parts = CommandFacts.getPartsForCommand("/nick dfowler password");

                var repository = new InMemoryRepository();
                var chatService = new ChatService(repository, new Mock<ICryptoService>().Object);
                var mockNotificationService = new Mock<INotificationService>();
                var command = new NickCommand(mockNotificationService.Object, repository, chatService);

                command.Handle(parts, "1", "lobby", "clientid", "test");
                ...
                mockNotificationService.Verify(m => m.OnUserCreated(user), Times.Once());
            }
        ...
        //public static void VerifyThrows<T>(ICommand command, string commandLine) where T : Exception
        //{
        //    var userId="1";
        //    var repository = new InMemoryRepository();
        //    var service = new ChatService(repository, new Mock<ICryptoService>().Object);
